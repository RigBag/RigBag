...
class UserController extends \ProtonLabs_Controller
{
	public function shortUrlAction( $hash ) {
    	...
    	return $this->render('ProtonRigbagBundle:Main:index.html.twig', array( 'adverts' => $adverts, 'userView' => $userView, 'lockInit' => true, 'ogpObject' => $ogpObject, 'bodyClass' => 'my-profile', 'user' => $user, 'isLoged' => $isLoged, 'userId' => $userId ) );
    	...
    	if ($request->isXmlHttpRequest()) {
	    	...
	    	switch( $action ) {
	    		case 'add':
	    			...
	    			if( $location ) {
	    				$circle		= $em->getRepository( 'ProtonRigbagBundle:Circle')->findOneBy( array( 'interest_id' => $sport->getId(), 'location_id' => $location->getId() ) );
